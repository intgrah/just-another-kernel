...
pub fn checkDeclarInfo(self: *TypeChecker, d: *const Declar) Reject!void {
    ...
    const ty_ty = try inferV(self, 0, value.envEmpty(), value.ctxEmpty(), info.ty, .Check);
    const sort = try ensureSortV(self, 0, ty_ty);
    if (d.* == .theorem) {
...
pub fn checkDefLike(self: *TypeChecker, d: *const Declar) Reject!void {
    ...
    const val_ty = try inferV(self, 0, value.envEmpty(), value.ctxEmpty(), val, .Check);
    const declared = eval.eval(self, 0, value.envEmpty(), d.info().ty);
    if (!conv.defEqCoreV(self, 0, val_ty, declared)) {
        return tc.reject("def_eq failed", .{});
    }
}

pub fn ensureSortV(self: *TypeChecker, depth: u32, v: V) Reject!LevelPtr {
...
fn inferSortOfV(self: *TypeChecker, depth: u32, e: E, c: C, ex: ExprPtr, comptime flag: InferFlag) Reject!LevelPtr {
    const t = try inferV(self, depth, e, c, ex, flag);
    return ensureSortV(self, depth, t);
}
...
pub const CachedInfer = struct {
    ...
    pub fn pack(v: V, is_checked: bool) CachedInfer {
        return .{ .bits = @intFromPtr(v) | @intFromBool(is_checked) };
    }

    pub fn result(self: CachedInfer) V {
        return @ptrFromInt(self.bits & ~@as(usize, 1));
    }

    pub fn checked(self: CachedInfer) bool {
...
pub fn inferV(self: *TypeChecker, depth: u32, e: E, c: C, ex: ExprPtr, comptime flag: InferFlag) Reject!V {
    ...
    if (self.tc_cache.infer_v.get(key)) |cached| {
        if (flag == .InferOnly or cached.checked()) {
            return cached.result();
        }
    }
    const r = switch (ex.asRef().kind) {
        .app => try inferAppV(self, depth, e, c, ex, flag),
        .lambda => |l| blk: {
            const dom = argValue(self, depth, e, l.binder_type);
            if (flag == .Check) {
                _ = try inferSortOfV(self, depth, e, c, l.binder_type, flag);
                ...
                _ = try inferV(self, depth + 1, e2, c2, l.body, flag);
            }
        ...
        .pi => |p| blk: {
            const l1 = try inferSortOfV(self, depth, e, c, p.binder_type, flag);
            const dom = argValue(self, depth, e, p.binder_type);
            const fresh = eval.mkBvarHc(self, depth, dom);
            const e2 = value.envExtend(self.arena, e, fresh);
            const c2 = value.ctxExtend(self.arena, c, dom);
            const l2 = try inferSortOfV(self, depth + 1, e2, c2, p.body, flag);
        ...
        .let => |l| blk: {
            ...
            if (flag == .Check) {
                _ = try inferSortOfV(self, depth, e, c, d.binder_type, flag);
                const val_ty = try inferV(self, depth, e, c, d.val, flag);
            ...
            const c2 = value.ctxExtend(self.arena, c, dom);
            break :blk try inferV(self, depth, e2, c2, d.body, flag);
        },
        .proj => |p| try inferProjV(self, depth, e, c, p.ty_name, p.idx, p.structure, flag),
        else => unreachable,
    };
    self.tc_cache.infer_v.put(util.smp_allocator, key, CachedInfer.pack(r, flag == .Check)) catch util.oom();
    return r;
}

fn inferAppV(self: *TypeChecker, depth: u32, e: E, c: C, ex: ExprPtr, comptime flag: InferFlag) Reject!V {
    const ua = expr.unfoldAppsStack(self.ctx.bump, ex);
    var args = ua.args;
    defer args.deinit(self.ctx.bump);
    var fty = try inferV(self, depth, e, c, ua.fun, flag);
    while (args.pop()) |arg| {
        const fty_f = eval.forceAll(self, depth, fty);
        switch (fty_f.*) {
            .pi => |p| {
                if (flag == .Check) {
                    const arg_ty = try inferV(self, depth, e, c, arg, flag);
...
fn inferProjV(
    ...
    _ = ty_name;
    const struct_ty = try inferV(self, depth, e, c, structure, flag);
