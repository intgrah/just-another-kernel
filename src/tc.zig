...
const infer_v = @import("infer.zig");
...
pub const TcCache = struct {
    ...
    infer_v: swiss_map.FxHashMap(struct { usize, ExprPtr }, infer_v.CachedInfer) = .empty,
...
fn checkDeclarWith(self: *const ExportFile, d: *const Declar, ar: *Arena, ctx: *TcCtx, cache: *TcCache) void {
    ...
    switch (d.*) {
        .theorem, .definition, .opaque_ => infer_v.checkDefLike(&checker, d) catch failWithName(d),
        .axiom, .constructor, .recursor => infer_v.checkDeclarInfo(&checker, d) catch failWithName(d),
