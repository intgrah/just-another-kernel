...
pub fn envExtendHc(self: *TypeChecker, parent: E, v: V) E {
...
pub fn truncEnv(self: *TypeChecker, e: E, k: u16) E {
    if (e.len <= k) return e;
    if (k == 0) return lsubBase(self, e.lsub);
    if (e.frame) |f| {
        const keep = f.mask & ((@as(u64, 1) << @intCast(k)) - 1);
        return frameEnv(self, internFrame(self, keep, f.slots[0..@popCount(keep)], f.lsub));
    }
    if (e.trunc_k == k) return e.trunc_r;
    const r = envExtendHc(self, truncEnv(self, e.parent, k - 1), e.v);
    const m = @constCast(e);
    m.trunc_k = k;
    m.trunc_r = r;
    return r;
...
pub fn applyClosure(self: *TypeChecker, depth: u32, clo: *const Closure, v: V, binder_ty: ?V) V {
    ...
    switch (clo.kind) {
        ...
        .infer => {
            ...
            return infer.inferV(self, depth, e, c, clo.body, .InferOnly) catch @panic("infer failed in closure application");
        },
    }
}
